...
// Linear Search untuk judul
int linearSearchJudul(Buku *buku, int n, string cari)
{
    for (int i = 0; i < n; i++)
    {
...
// Binary Search untuk ID angka
int binarySearchId(Buku *buku, int n, int cariId)
{
    ...
    while (kiri <= kanan)
    {
        ...
        if (buku[tengah].id == cariId)
            return tengah;
        else if (buku[tengah].id < cariId)
            kiri = tengah + 1;
        else
            kanan = tengah - 1;
    }
...
void menuSearching(Buku *buku, int jumlahData)
{
    int pilihCari;
    int hasil = -1;
    system("cls");
    ...
    {
        int idCari;
        sortIdAsc(buku, jumlahData);
        cout << "\nMasukkan ID buku(Harus Angka): ";
        cin >> idCari;
        hasil = binarySearchId(buku, jumlahData, idCari);
        break;
    }
    ...
    {
        string judulCari;
        cout << "\nMasukkan judul buku(Harus String): ";
        getline(cin, judulCari);
        hasil = linearSearchJudul(buku, jumlahData, judulCari);
        break;
    }
    ...
    {
        cout << "\nData ditemukan:\n";
        cout << "ID      : " << buku[hasil].id << endl;
        cout << "Judul   : " << buku[hasil].judul << endl;
        cout << "Penulis : " << buku[hasil].penulis << endl;
        cout << "Genre   : " << buku[hasil].genre << endl;
        cout << "Rating  : " << buku[hasil].detail.rating << endl;
        cout << "Reviewer: " << buku[hasil].detail.reviewer << endl;
        cout << "Review  : " << buku[hasil].detail.teksReview << endl;
    }
    else
    {
...
int main()
{
    ...
    Buku daftarBuku[] =
        {
            {101, "Harry Potter", "J.K Rowling", "Fantasy", {"Seru banget", 5, "Budi"}},
            {102, "Solo Leveling", "Chugong", "Action", {"Overpowered tapi keren", 4.5, "Andi"}},
            {103, "Lord of the Rings", "Tolkien", "Fantasy", {"Klasik epic", 5, "Rina"}},
            {104, "The Hobbit", "Tolkien", "Fantasy", {"Petualangan santai", 4, "Sari"}},
            {105, "Attack on Titan", "Isayama", "Action", {"Plot twist gila", 5, "Doni"}},
            {106, "Death Note", "Tsugumi Ohba", "Thriller", {"Mind game keren", 4.8, "Rizky"}},
            {107, "Naruto", "Masashi Kishimoto", "Action", {"Nostalgia", 4.2, "Fajar"}},
            {108, "One Piece", "Oda", "Adventure", {"Dunia luas banget", 5, "Asep"}},
            {109, "Tokyo Ghoul", "Ishida", "Dark", {"Agak sedih", 4, "Dewi"}},
            {110, "Demon Slayer", "Gotouge", "Action", {"Visual bagus", 4.3, "Lina"}}};
    ...
    do
    {
        ...
        case 6:
            menuSearching(daftarBuku, jumlahData);
            cin.ignore();
            cin.get();
            break;
        }
